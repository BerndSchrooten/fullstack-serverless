 ...
 */
function uploadDirectory(aws, bucketName, bucketPrefix, clientRoot, headerSpec) {
    ...
    return BbPromise.all(
        uploadList.map(u => {
            return uploadFile(aws, bucketName, bucketPrefix, u.filePath, u.fileKey, u.headers);
        })
    );
}
 ...
 * @param {string} clientRoot - Full path to the root directory of client files
 */
function uploadFile(aws, bucketName, bucketPrefix, filePath, fileKey, headers) {
    let baseHeaderKeys = [
        'Cache-Control',
        'Content-Disposition',
        'Content-Encoding',
    ...
    let params = {
        Bucket: bucketName,
        Key: bucketPrefix ? `${bucketPrefix.substring(1)}/${fileKey}` : fileKey,
        Body: fileBuffer,
