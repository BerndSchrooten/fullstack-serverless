 ...
 */
function listObjectsInBucket(aws, bucketName, bucketPrefix) {
    const params = {
        Bucket: bucketName
    };

    if (bucketPrefix) {
        params.Prefix = bucketPrefix;
    }
 ...
 * @param {Object} aws - AWS class
 * @param {string} bucketName - Name of bucket to be deleted
 * @param {string} bucketPrefix - Prefix of the files to be deleted
 */
function emptyBucket(aws, bucketName, bucketPrefix) {
    return listObjectsInBucket(aws, bucketName, bucketPrefix).then(resp => {
